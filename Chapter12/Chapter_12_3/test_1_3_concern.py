...
class TestAddCart(object):
    ...
    def test_add_cart(self,book_name,author,target,storm):
        ...
        # 单击加入收藏按钮
        storm.find_element(AppiumBy.IOS_CLASS_CHAIN, '**/XCUIElementTypeButton[`label == "收藏"`][2]').click()
        ...
        # 点击"我的"
        storm.find_element(AppiumBy.IOS_CLASS_CHAIN, '**/XCUIElementTypeButton[`label == "我的"`]').click()
        # 点击商品收藏，进入收藏详情列表
        storm.find_element(AppiumBy.IOS_PREDICATE, 'label == "商品收藏"').click()
        ...
        # 将商品取消收藏
        # 左滑商品
        # storm.execute_script('mobile: swipe', {'direction': 'right'})
        book_ele = storm.find_element(AppiumBy.IOS_PREDICATE,
